...
void interactivemode(char*codetoload)
{
  ...
  for(;;)
  {
    ...
    {
      vm.userinput=(vm.userinput&0xff00ffff)|((e.key.keysym.sym&0xff)<<16);

      if(ui.opt_dumpkeys)
      {
        static int last=0;
        int now=getticks();
        if((!e.key.keysym.sym) && e.key.keysym.unicode)
             e.key.keysym.sym=e.key.keysym.unicode;
        printf("%d %d %d %d\n",now-last,e.key.keysym.sym,
          e.key.keysym.unicode,e.key.keysym.mod);
        last=now;
      }
      ...
      if(ui.osd_visible)
      {
        if(e.key.keysym.sym==SDLK_UP && (e.key.keysym.mod&KMOD_CTRL))
        {
          ed_increment(ed.cursor);
          codechanged=1;
        }
        else
        if(e.key.keysym.sym==SDLK_DOWN && (e.key.keysym.mod&KMOD_CTRL))
        {
          ed_decrement(ed.cursor);
          codechanged=1;
        }
        else
        if(e.key.keysym.sym==SDLK_LEFT && (e.key.keysym.mod&KMOD_CTRL))
        {
          ed_prev();
        }
        else
        if(e.key.keysym.sym==SDLK_RIGHT && (e.key.keysym.mod&KMOD_CTRL))
        {
          ed_next();
        }
        else
        ...
        {
          ed_left(e.key.keysym.mod&KMOD_SHIFT);
        }
        else
        if(e.key.keysym.sym==SDLK_RIGHT)
        {
          ed_right(e.key.keysym.mod&KMOD_SHIFT);
        }
        else
        if(e.key.keysym.sym==SDLK_UP)
        {
          ed_up(e.key.keysym.mod&KMOD_SHIFT);
        }
        else
        if(e.key.keysym.sym==SDLK_DOWN)
        {
          ed_down(e.key.keysym.mod&KMOD_SHIFT);
        }
        else
        ...
        if(e.key.keysym.sym=='c' && (e.key.keysym.mod&KMOD_CTRL))
        {
          ed_copy();
        }
        else
        if(e.key.keysym.sym=='k' && (e.key.keysym.mod&KMOD_CTRL))
        {
          ed_copy();
        }
        else
        if(e.key.keysym.sym=='v' && (e.key.keysym.mod&KMOD_CTRL))
        {
          ed_paste();
        }
        else
        if(e.key.keysym.sym=='x' && (e.key.keysym.mod&KMOD_CTRL))
        {
          ed_cut();
        }
        else
        if(e.key.keysym.sym=='a' && (e.key.keysym.mod&KMOD_CTRL))
        {
...
int main(int argc,char**argv)
{
  ...
  while(*argv)
  {
    ...
    } else
    {
      ...
      buflgt=fread(buf+hdrlgt,1,1024-hdrlgt,f);
      buf[hdrlgt+buflgt]='\0';
